...
A elaboração do diagrama de classes foi conduzida a partir de uma abordagem colaborativa e iterativa durante a [reunião de ajustes do diagrama de classes](/PadroesDeProjeto/3.5.2.ReunioesAtas.md), baseada na análise do modelo de domínio previamente definido para o sistema ConhecendoRequisitos. Inicialmente, os integrantes do grupo revisaram os requisitos funcionais e não funcionais do projeto, identificando as principais entidades do domínio, seus atributos e responsabilidades. Em seguida, essas entidades foram traduzidas em classes UML, respeitando os princípios da orientação a objetos, como encapsulamento, herança e separação de responsabilidades. As associações entre classes foram definidas com base nas relações semânticas do domínio, utilizando verbos explicativos para tornar os vínculos mais claros. O diagrama passou por revisões sucessivas em grupo, nas quais as decisões foram discutidas e validadas à luz das recomendações da professora e da literatura sobre modelagem UML, resultando em uma versão final mais coesa, legível e alinhada ao domínio do problema.

## Alterações

De acordo com as recomendações da professora, fizemos duas alterações principais no diagrama de classes, a primeira se consiste na escrita de verbos que ligam as classes umas as outras, com base no [modelo de domínio](/PadroesDeProjeto/3.5.4.ModeloDominio.md), através das palavras "contém" e "tem". Além disso, optamos por remover a classe "Edtatística", pois como discutido na reunião, ela não se aplicaria a uma classe.
...
Na reanálise do diagrama de classes, adotamos uma postura crítica para reconciliar o diagrama pré-existente com as recomendações da professora. Inserimos verbos de relacionamento ("contém", "tem") com base no [modelo de domínio](/PadroesDeProjeto/3.5.4.ModeloDominio.md), o que clarificou a semântica das associações e melhorou a legibilidade do modelo. Também optamos por remover a classe `Estatística`, entendendo que suas responsabilidades são melhor tratadas como dependências ou relatórios derivados das outras classes, e não como uma entidade de domínio independente.

As decisões sobre abstração e responsabilidades — por exemplo, a definição de `Usuario` como superclasse e as especializações `Aluno` e `Administrador` — foram tomadas coletivamente, priorizando a redução de classes redundantes e a coerência do modelo com o domínio. O processo foi iterativo: propusemos mudanças, discutimos em grupo, validamos contra o modelo de domínio e consolidamos o diagrama reanalisado antes de avançar para a implementação.

# Conclusão

A reanálise do diagrama de classes resultou em uma modelagem mais alinhada ao domínio e com maior clareza estrutural. A inclusão de verbos de ligação e a remoção da classe `Estatística` simplificaram o conjunto de entidades, deixando o diagrama mais coeso e fácil de interpretar pela equipe e por novos colaboradores. Essas alterações também facilitam a transição para a implementação, pois as responsabilidades das classes estão mais bem definidas e menos sobrepostas.

O diagrama reanalisado passa a servir como referência estável para as próximas etapas do projeto, suportando a implementação de trilhas, módulos, quizzes e mecanismos de progresso com maior consistência semântica.
...
| ------ | ----- | ---------------------- | ------------------------------------------------ | ---------------------------------------------------------- | ------------------- |
| 1.0    | 13/05 | Criação do documento   | [Yan Matheus](https://github.com/Yanmatheus0812) | [Isabella Choukaira](https://github.com/isabellachoukaira) | Documento criado    |
| 1.2    | 14/05 | adicionado metodologia | [Yan Aguiar](https://github.com/Yanmatheus0812)  | [Arthur Oliveira](https://github.com/arthurevang)          | Revisado e aprovado |
